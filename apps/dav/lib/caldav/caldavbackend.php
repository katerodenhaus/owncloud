...
class CalDavBackend extends AbstractBackend implements SyncSupport, SubscriptionSupport, SchedulingSupport
{
     ...
     * @param Principal     $principalBackend Backend for the principal
     */
    public function __construct(IDBConnection $db, Principal $principalBackend)
    {
        $this->db               = $db;
        $this->principalBackend = $principalBackend;
        $this->sharingBackend   = new Backend(
            $this->db,
            $principalBackend,
            'calendar'
        );
     ...
     *
     * @param string $principalUri User's URI
     *
     * @return array
    ...
    public function getCalendarsForUser($principalUri)
    {
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            ...
            $calendar = [
                ...
                '{' . Plugin::NS_CALDAV . '}supported-calendar-component-set' =>
                    new SupportedCalendarComponentSet($components),
                '{' . Plugin::NS_CALDAV . '}schedule-calendar-transp'         =>
                    new ScheduleCalendarTransp($row['transparent'] ? 'transparent' : 'opaque'),
            ];
        ...
        $principals   = $this->principalBackend->getGroupMembership(
    ...
    public function updateCalendar($calendarId, \Sabre\DAV\PropPatch $propPatch)
    {
        ...
        $propPatch->handle(
            $supportedProperties,
            function ($mutations) use ($calendarId) {
            $newValues = [];
    ...
    public function getCalendarObjectByUri($objectUri)
    {
        ...
        $query->select(['id', 'uri', 'lastmodified', 'etag', 'calendarid', 'size', 'calendardata', 'componenttype'])
              ...
              ->andWhere(
                  $query->expr()->eq(
                      'uri',
                      $query->createNamedParameter($objectUri)
                  )
              );

        if (\OC::$server->getConfig()->getSystemValue(
            'encrypt_cal',
            false
        )
        ) {
    ...
    /**
     * Gets all of the event's data in a human-readable format
     *
     * @param string $calendarData The iCal Calendar Data
     *
     * @return array
     * @throws BadRequest
     */
    public function getAllEventData($calendarData)
    {
        ...
        return [
            ...
            'firstOccurence' => $firstOccurence === null ? null : max(
                0,
                $firstOccurence
            ),
    ...
    private function notifyCalendarPrincipal($calendarId, $message_text)
    {
        ...
        if (\OC::$server->getConfig()->getUserValue($principal, 'calendar', 'reminderHipchat') === 'on') {
            ...
            if ($messenger !== null) {
                ...
                $messenger->sendUserMessage($message, $email);
            }
        }
    }
    ...
    /**
     * @param string $principal   Calendar owner
     * @param string $displayName Display name
     *
     * @return array|null
     */
    public function getCalendarByName($principal, $displayName)
    {
        $principal = $this->convertPrincipal($principal, true);
        $fields    = array_values($this->propertyMap);
        $fields[]  = 'id';
        $fields[]  = 'uri';
        $fields[]  = 'synctoken';
        ...
        $calendar = [
            ...
            'uri'          => $row['uri'],
            'principaluri' => $row['principaluri'],
            'getctag'      => 'http://sabre.io/ns/sync/' . ($row['synctoken'] ? $row['synctoken'] : '0'),
    ...
    {
        $fields   = array_values($this->subPropertyMap);
        $fields[] = 'id';
        $fields[] = 'uri';
        $fields[] = 'source';
        $fields[] = 'principaluri';
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            ...
            foreach ($this->subPropertyMap as $xmlName => $dbName) {
        ...
        $propPatch->handle(
            ...
            foreach ($mutations as $propertyName => $propertyValue) {
                if ($propertyName === '{http://calendarserver.org/ns/}source') {
                    ...
                    $fieldName             = $this->subPropertyMap[$propertyName];
                    $newValues[$fieldName] = $propertyValue;
                }
            }
