...
use Doctrine\Common\CssEncrypt;
...
class CalDavBackend extends AbstractBackend implements SyncSupport, SubscriptionSupport, SchedulingSupport
{
    ...
    private
    function convertPrincipal($principalUri, $toV2)
    {
     ...
     */
    protected
    function addChange($calendarId, $objectUri, $operation)
    {
    ...
    {
        file_put_contents('/var/tmp/debug', "TEST3", FILE_APPEND);
        $query = $this->db->getQueryBuilder();
    ...
    private function decryptColumns($query)
    {
        if (\OC::$server->getConfig()->getAppValue('core', 'encrypt_cal')) {
            $decryptQuery = new CssEncrypt($query);
    ...
    function createCalendarObject($calendarId, $objectUri, $calendarData)
    {
        ...
        $query->insert('calendarobjects')
            ->values([
                ...
                'calendardata'   => $query->createFunction("AES_ENCRYPT('$calendarData', 'cssLoginKey')"),
     ...
     */
    public
    function updateShares($shareable, $add, $remove)
    {
     ...
     */
    public
    function applyShareAcl($resourceId, $acl)
    {
        return $this->sharingBackend->applyShareAcl($resourceId, $acl);
    }

    private function encryptColumns($query)
    {
        if (\OC::$server->getConfig()->getAppValue('core', 'encrypt_cal')) {
            $decryptQuery = new CssEncrypt($query);

            return $decryptQuery->encryptData();
        } else {
            return $query;
        }
    }
