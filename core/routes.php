...
$application->registerRoutes(
    $this, [
             'routes' => [
                 ...
                 // Calendars
                 ['name' => 'calendar#getCalendarUsers', 'url' => '/v1/calendar/users', 'verb' => 'GET'],
                 ['name' => 'calendar#getUserEvents', 'url' => '/v1/calendar/user/{user}/events/{range}', 'verb' => 'GET'],
                 [
                     'name' => 'calendar#bookUserEvent',
                     'url'  => '/v1/calendar/user/{user}/calendar/{calendar}/start/{start}/end/{end}/link/',
                     'verb' => 'POST'
                 ],
                 [
                     'name' => 'calendar#getUserCalendarByName',
                     'url'  => '/v1/calendar/user/{user}/calendar/{name}', 'verb' => 'GET'
                 ],
                 [
...
// Sharing routes
$this->create('files_sharing.sharecontroller.showShare', '/s/{token}')->action(
    function ($urlParams) {
        $app = new \OCA\Files_Sharing\AppInfo\Application($urlParams);
        $app->dispatch('ShareController', 'showShare');
    }
);
$this->create('files_sharing.sharecontroller.authenticate', '/s/{token}/authenticate')->post()->action(
    function ($urlParams) {
        $app = new \OCA\Files_Sharing\AppInfo\Application($urlParams);
        $app->dispatch('ShareController', 'authenticate');
    }
);
$this->create('files_sharing.sharecontroller.showAuthenticate', '/s/{token}/authenticate')->get()->action(
    function ($urlParams) {
        $app = new \OCA\Files_Sharing\AppInfo\Application($urlParams);
        $app->dispatch('ShareController', 'showAuthenticate');
    }
);
$this->create('files_sharing.sharecontroller.downloadShare', '/s/{token}/download')->get()->action(
    function ($urlParams) {
        $app = new \OCA\Files_Sharing\AppInfo\Application($urlParams);
...
// used for heartbeat
$this->create('heartbeat', '/heartbeat')->action(
    function () {
        // do nothing
    }
);
